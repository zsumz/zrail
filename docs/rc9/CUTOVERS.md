...
| `tests/guardrails/release_graph.rs` | Twelve declaration, raw-construction and required-input assertions retain repeated 144-case evidence in `kafka-driver.retired.fragment.toml`. Its file-only tree selectors miss special entries. The separate `kafka-driver.retired-boundary.fragment.toml` corrects those seven rules; deterministic entry-error and repeated corrected-policy artifact qualification remain open. Keep the checker installed. |
