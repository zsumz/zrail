...
| `tests/guardrails/transport_authority.rs` | The written-method inventory and its detector/parser assertions are verified by `rust:inventory:kd-transport-methods`. Close the other eight `KD-TRANSPORT-*` assertions and qualify the assembled bundle before removal. Preserve function-value `ExprPath` counts, written renames, method quantities, and exact implementing-type/trait sets. |
