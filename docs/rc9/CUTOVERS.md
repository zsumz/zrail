...
| `tests/guardrails/release_graph.rs` | Twelve declaration, raw-construction and required-input assertions now have repeated 144-case committed evidence in `kafka-driver.retired.fragment.toml`. The seven retired-tree predicates still require read-error, entry-error and symlink boundary evidence; keep the checker installed. |
...
| `src/reactor/direct_plaintext/cluster_runtime/route_state_test.rs` | Six verified `repository:file:kd-route-*` policies replace only `route_modules_cannot_own_a_selector_or_legacy_routing_capability` and its compile inputs after full qualification. Retain the mixed file and its fifteen `KD-ROUTE-RUNTIME-*` assertions; their execution receipt remains open. |
