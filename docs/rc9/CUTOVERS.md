...
| `tests/guardrails/release_metadata.rs` | `kafka-driver.metadata.fragment.toml` supplies 35 native policies with 33 field/read assertion IDs plus the separately bound fixed-parent precondition verified. Preserve the exact three manifest-to-license mappings, authored inheritance, ordered publication lists, UTF-8 license equality, required files and deliberate raw markers. Full repository qualification is still required before removal; the parent proof adds no new license authority. |
