...
| `tests/guardrails/transport_authority.rs` | The written-method and expression-path quantities and their detector/parser assertions are verified by `rust:inventory:kd-transport-methods` and `rust:inventory:kd-transport-expression-paths`. Close the remaining six `KD-TRANSPORT-*` assertions and qualify the assembled bundle before removal. Preserve function-value `ExprPath` counts, written renames, method quantities, and exact implementing-type/trait sets. |
