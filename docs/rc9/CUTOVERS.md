...
| `tests/guardrails/transport_authority.rs` | Ten assertions are verified by `rust:inventory:kd-transport-methods`, `rust:inventory:kd-transport-expression-paths`, `rust:inventory:kd-transport-owners`, and `rust:inventory:kd-transport-renames`. Close the two remaining trait-implementation assertions and qualify the assembled bundle before removal. The native trait-implementation fragment is implemented but not yet verified. Preserve function-value `ExprPath` counts, written renames, method quantities, and exact implementing-type/trait sets. |
