...
| `KD-TRANSPORT-DETECTOR-IMPLS` | Native written trait inventories retain both trait and path-type suffixes, explicit per-trait type filters, exact ownership sets and independent quantities. Full frozen-scope differential evidence remains required before verification. |
| `KD-TRANSPORT-DETECTOR-RENAMES` | Native explicit-import-rename inventories preserve source and alias identities, quantities and exact owner sets. Full frozen-scope positive/negative differential evidence remains required before verification. |
| `KD-TRANSPORT-IMPLS` | Native written trait inventories retain both trait and path-type suffixes, explicit per-trait type filters, exact ownership sets and independent quantities. Full frozen-scope differential evidence remains required before verification. |
| `KD-TRANSPORT-RENAMES` | Native explicit-import-rename inventories preserve source and alias identities, quantities and exact owner sets. Full frozen-scope positive/negative differential evidence remains required before verification. |
