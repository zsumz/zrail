...
pub(super) fn detector_source() -> String {
    let path = project()
        .join("crates/zrail-testkit/tests/fixtures/rc9/transport_methods/transport_authority.rs");
    let source = fs::read_to_string(path).expect("bound original detector source");
    assert_eq!(
...
pub(super) enum LegacyMeasure {
    #[serde(rename = "distinct-owner-files")]
    OwnerFiles,
    #[serde(rename = "distinct-rename-identities")]
    RenameIdentities,
    #[serde(rename = "distinct-trait-impl-identities")]
    TraitImplIdentities,
}
