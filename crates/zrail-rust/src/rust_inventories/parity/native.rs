...
pub(super) fn observed(root: &Path, source: &str) -> BTreeMap<String, usize> {
    observed_with_subject(
        root,
        source,
        "kind='written-methods',names=['turn_component','poll_io','wake_handle','pulse_handle']",
    )
}

pub(super) fn observed_with_subject(
    root: &Path,
    source: &str,
    subject: &str,
