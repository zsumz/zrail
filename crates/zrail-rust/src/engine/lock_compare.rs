...
use zrail_core::{
    Contract, DependencyMode, Finding, FindingSink, LockFile, LockedDependencyKind,
    LockedDependencyScope, LockedPackage,
};
    ...
    if current.engine != env!("CARGO_PKG_VERSION") {
        findings.push(
            Finding::error(
                ...
                "lock",
                format!(
                    "zrail.lock was generated by engine {}, current engine is {}",
                    current.engine,
                    env!("CARGO_PKG_VERSION")
                ),
            )
            .with_help("run `zrail diff` before regenerating the lock with `zrail update`"),
        );
    }
