...
from rc9_lock_array_common import PREFIX, SUITES, execution


class LockArrayRunner(unittest.TestCase):
    def test_complete_suite_scope(self):
        self.assertEqual(len(SUITES), 5)
        self.assertEqual([row[2] for row in SUITES.values()], [0, 24, 5, 5, 24])
    ...
    def test_exact_execution_rejects_skips_duplicates_and_wrong_counts(self):
        for short in SUITES:
            name = PREFIX + short
            output = (f"\nrunning 1 test\ntest {name} ... ok\n\n"
                      "test result: ok. 1 passed; 0 failed; 0 ignored; 0 measured; 643 filtered out; finished in 0.10s\n").encode()
            self.assertEqual(execution(output, 0, name, 644)["passed"], 1)
            for value in [output.replace(b"... ok", b"... ignored"), output.replace(name.encode(), b"unrelated"),
                          ...
                          output.replace(b"643 filtered", b"642 filtered"), output + output]:
                with self.assertRaises(ValueError):
                    execution(value, 0, name, 644)
            with self.assertRaises(ValueError):
                execution(output, 1, name, 644)
