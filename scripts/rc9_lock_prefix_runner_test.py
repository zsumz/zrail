...
class LockPrefixRunner(unittest.TestCase):
    ...
    def test_exact_execution_rejects_skips_duplicates_and_wrong_counts(self):
        for short in SUITES:
            name = PREFIX + short
            output = (f"\nrunning 1 test\ntest {name} ... ok\n\n"
                      "test result: ok. 1 passed; 0 failed; 0 ignored; 0 measured; 653 filtered out; finished in 0.10s\n").encode()
            self.assertEqual(execution(output, 0, name, 654)["passed"], 1)
            for value in [output.replace(b"... ok", b"... ignored"), output.replace(name.encode(), b"unrelated"),
                          ...
                          output.replace(b"653 filtered", b"652 filtered"), output + output]:
                with self.assertRaises(ValueError):
                    execution(value, 0, name, 654)
            with self.assertRaises(ValueError):
                execution(output, 1, name, 654)
